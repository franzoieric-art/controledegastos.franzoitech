// js/app.js - VERSÃO DEFINITIVA (29/08/2025)
...
function saveState() {
    ...
    state.saveTimeout = setTimeout(async () => {
        ...
        // Salva apenas os dados do mês ativo para ser mais eficiente
        await updateUserField(state.currentUserId, `monthlyData.${state.activeMonthIndex}`, state.monthlyData[state.activeMonthIndex]);
        showSaveFeedback();
    }, 1500);
}
...
function recalculateAndDisplayTotals(m) {
    ...
    const t = {
        pj: (d.pjEntries || []).reduce((s, e) => s + (e.amount || 0), 0),
        pf: (d.pfEntries || []).reduce((s, e) => s + (e.amount || 0), 0),
        personal: (d.expenses || []).flat().reduce((a, day) => a + (day.personalEntries || []).reduce((s, e) => s + (e.amount || 0), 0), 0),
        business: (d.expenses || []).flat().reduce((a, day) => a + (day.businessEntries || []).reduce((s, e) => s + (e.amount || 0), 0), 0)
    };
...
function showMonth(monthIndex) {
    ...
    if(state.chartInstances) Object.values(state.chartInstances).forEach(c => c?.destroy());
...
async function handleLogin(user) {
    try {
        ...
        alert("Não foi possível carregar seus dados. Por favor, verifique sua conexão e tente novamente.");
    }
}
...
function initializeAppUI() {
    ...
    constants.monthNames.forEach((_, index) => {
        ...
        monthContentContainer.insertAdjacentHTML('beforeend', html);
    });
...
function bindGlobalEventListeners() {
    ...
    document.getElementById('theme-toggle-btn').addEventListener('click', () => {
        ...
        applyTheme(newTheme);
        if(state.currentUserId) showMonth(state.activeMonthIndex);
    });
    ...
    document.body.addEventListener('click', (event) => {
        ...
        if (target.closest('.add-entry-btn')) {
            ...
            if (type === 'pj') {
                ...
                newEntry = { id: Date.now(), description: '', amount: 0, paymentMethod: 'Pix', card: '', category: (state.categories[0] || {}).name || 'Outros' };
                state.monthlyData[m].expenses[day][`${category}Entries`].push(newEntry);
                render.renderExpenseTable(m);
            }
        ...
        if (target.closest('.remove-btn')) {
            ...
            if (type === 'pj') state.monthlyData[m].pjEntries = state.monthlyData[m].pjEntries.filter(e => e.id !== id);
            else if (type === 'pf') state.monthlyData[m].pfEntries = state.monthlyData[m].pfEntries.filter(e => e.id !== id);
            else state.monthlyData[m].expenses[day][`${category}Entries`] = state.monthlyData[m].expenses[day][`${category}Entries`].filter(e => e.id !== id);
